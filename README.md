...
## HTTPS
- Build with `-DUSE_TLS`.
- Pass `verifyCert=true` to `begin()` to enforce CA validation (Let’s Encrypt roots shipped in `Certs.h`).
- Pass `verifyCert=false` to skip validation (still uses HTTPS transport).
